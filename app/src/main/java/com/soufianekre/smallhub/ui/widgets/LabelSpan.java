...
public class LabelSpan extends ReplacementSpan {
    ...
    private void setupFontMetrics(CharSequence text, int start, int end, Paint.FontMetricsInt fm, Paint p) {
        txtPaint.set(p);
    ...
    @Override public void draw(@NonNull Canvas canvas, CharSequence text, int start, int end,
        ...
        if (bgWidth == maxWidth) {
            text = TextUtils.ellipsize(text.subSequence(start, end).toString(),
                    txtPaint, bgWidth - 2 * paddingW, TextUtils.TruncateAt.MIDDLE);
            start = 0;
            end = text.length();
        }
